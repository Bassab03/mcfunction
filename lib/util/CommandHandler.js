 ...
 */
function getFormattedOutput(options) {
  const output = [];
  for (const option of options) {
    const result = option.reformatOutput();
...
function CommandHandler(tokens, editor, bufferPosition) {
  ...
  if (bodyHasChanged(tokens)) {
    ...
    return getFormattedOutput(currentOutput); // TODO: map to actual output object
  }
  ...
  for (const commandData of commands) {
    ...
    if (isWithinCommand) {
      ...
      return getFormattedOutput(currentOutput);
    } else {
