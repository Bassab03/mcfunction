 ...
 * 
 * @param {CommandBuilder} command The CommandBuilder currently being used
 * @param {BaseOption[]} output The output being checked
 * @param {String} token The current token
 * @returns {CommandBuilder|Section} The section/builder to use
 */
function updateCurrentCommand(command, output, token) {
  return output.find((opt) => {
  ...
  */
  if (bodyHasChanged(tokens)) {
    ...
    if (tokens.length !== currentLine.length) {
      ...
      currentCommand = updateCurrentCommand(currentCommand, currentOutput, tokens[tokens.length - 2]);
  ...
  for (const commandData of commands) {
    ...
    if (isWithinCommand) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        const oldCommand = currentCommand;
        if (val.done) {return;}
        currentCommand = updateCurrentCommand(currentCommand, val.value, tokens[i]);
