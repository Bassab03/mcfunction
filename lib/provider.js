		...
		if(! lineText.includes(" ")) {
			...
			if(stop["type"] == "command") out = this.getCommandOption(lastText);
				...
				for(var ench of enchantments) if(ench.startsWith(lastText)) out.push({
					text: ench,
					type: "enchantment",
					iconHTML: "<img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/enchantment.svg\">"
				})
			} else if(stop["type"] == "item") {
				for(var item of items) if(item.startsWith(lastText)) out.push({
					text: item,
					type: "item",
					iconHTML: "<img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/item.svg\">"
				});
			} else if(stop["type"] == "recipe") {
				for(var recipe of recipes) if(recipe.startsWith(lastText)) out.push({
					text: recipe,
					type: "recipe",
					iconHTML: "<img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/recipe.svg\">"
				});
