	...
	getSuggestions: function (args) {
		...
		if(! lineText.includes(" ")) {
			...
			if(stop["type"] == "command") out = this.getCommandOption(lastText);
				...
				for(var block of res.id.blocks) if(block.startsWith(lastText)) out.push({
					text: block,
					type: "block",
					iconHTML: "<img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/block.svg\">"
				});
			} else if(stop["type"] == "effect") {
				for(var effect of res.id.effects) if(effect.startsWith(lastText)) out.push({
					text: effect,
					type: "effect",
					iconHTML: "<i class=\"icon effect\">e</i>"
				});
			} else if(stop["type"] == "advancement") {
				for(var adv of res.id.advancements) if(adv.startsWith(lastText)) out.push({
					text: adv,
					type: "advancement",
					iconHTML: "<i class=\"icon advancement\">a</i>"
				});
			} else if(stop["type"] == "enchantment") {
				for(var ench of res.id.enchantments) if(ench.startsWith(lastText)) out.push({
					text: ench,
					type: "enchantment",
				...
				for(var ent of res.id.entities) if(ent.startsWith(lastText)) out.push({
					text: ent,
					type: "entity-id",
					iconHTML: "<i class=\"icon entity\">a</i>"
				});
			} else if(stop["type"] == "item") {
				for(var item of res.id.items) if(item.startsWith(lastText)) out.push({
					text: item,
					type: "item",
					iconHTML: "<i class=\"icon item\" ><img style=\"width:1.5em; height:1.5em;\" src=\"" + __dirname + "/svgicon/item.svg\"></i>"
				});
			} else if(stop["type"] == "recipe") {
				for(var recipe of res.id.recipes) if(recipe.startsWith(lastText)) out.push({
					text: recipe,
					type: "recipe",
				...
				for(var slot of res.id.slots["inventory"]) if(slot.startsWith(lastText)) out.push({
					text: slot,
					type: "slot",
					iconHTML: "<i class=\"icon slot\">s</i>"
				});
			} else if(stop["type"] == "objective-slot") {
				for(var slot of res.id.slots["objective"]) if(slot.startsWith(lastText)) out.push({
					text: slot,
					type: "slot",
					iconHTML: "<i class=\"icon slot\">s</i>"
				});
