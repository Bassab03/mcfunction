...
const selectors = require("./id/selectors.json");
...
module.exports = {
  ...
  getOutput: function (lastText, stop) {
    let out = [];
    switch(stop.type){
      case "command":
  ...
  getSelectorOption: function (text) {
    let out = [];
    const selectorText = text.match(/(?<=([@]|[a-z]+)\[).*/)[0];
    if(selectorText.split("{").length % 2 == selectorText.split("}").length % 2){ // closed
      ...
      for(let sel of selectors){
        if(/=/g.test(lastText) && lastText.split("=")[0] == sel.name){ // use value
          out = this.getOutput(lastText.split("=")[1],sel);
          break;
        }else if(sel.name.startsWith(lastText)){
          out.push({
            text: sel.name,
            type: "selector",
