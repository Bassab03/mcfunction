...
module.exports = {
  ...
  runCycle: function(args, cycle) {
    ...
    for(; i < args.length; ) {
      ...
      if (realStop.type == "option" && realStop.change != null && realStop.change[arg] != null) {
        ...
        if(args[0] == "block" || args[0] == "minecraft:block" || args[0] == "item" || args[0] == "minecraft:item"){
          i+=2;
          c+=1;
  ...
  getOutput: function (lastText, stop) {
    ...
    function shouldPrependMC(data) {
      if(lastText.startsWith("minecraft:")){
        return data.replace("minecraft:","");
      }else{
        return data;
      }
    }
