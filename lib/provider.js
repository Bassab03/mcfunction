scoreconst commands = require("./commands.json");
...
let slots = require("./id/slot.json");
...
for(let cat in slots){
	for(let item in slots[cat]){
		const tmp = [];
		const original = slots[cat][item];
		if(/<item>/gm.test(original)){
			for(let it of items){
				tmp.push(original.replace(/<item>/gm,items[it]));
			}
		}else if(/<block>/gm.test(original)){
			for(let it of blocks){
				tmp.push(original.replace(/<block>/gm,blocks[it]));
			}
		}else if(/<entity>/gm.test(original)){
			for(let it of entities){
				tmp.push(original.replace(/<entity>/gm,entities[it]));
			}
		}
		slots[cat].concat(tmp);
	}
	slots[cat].sort();
}
