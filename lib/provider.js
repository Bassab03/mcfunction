	...
	getSuggestions: function (args) {
		...
		if(!lineText.includes(" ")) {
			...
			if(stop["type"] == "command") out = this.getCommandOption(lastText);
				...
				for(var loot of loots) if(loot.startsWith(lastText) || loot.replace("minecraft:","").startsWith(lastText)) out.push({
					text: loot,
					type: "loot",
					iconHTML: "<i class=\"icon loot\">LT</i>"
				});
			} else if(stop["type"] == "recipe") {
				for(var recipe of items) if(recipe.startsWith(lastText) || recipe.replace("minecraft:","").startsWith(lastText)) out.push({
					text: recipe,
					type: "recipe",
