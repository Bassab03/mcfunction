...
// copies
const blocks2 = require("./id/block.json");
const effects2 = require("./id/effect.json");
const advancements2 = require("./id/advancement.json");
const enchantments2 = require("./id/enchantment.json");
const items2 = require("./id/item.json");
const entities2 = require("./id/entity.json");
const biomes2 = require("./id/biomes.json");
		...
		if(!lineText.includes(" ")) {
			...
			if(stop.type == "command") {
				...
				for(var opt of stop.value) {
					// stuff
					if(typeof(opt) != "string"){ // assuming object
						if(){
				...
				for(var adv of advancements) {
					if(adv.startsWith(lastText) || adv.replace("minecraft:","").startsWith(lastText)) {
						out.push({
							text: adv,
							type: "advancement",
			...
			} else if(stop.type == "loot") {
				for(var loot of loots) {
					if(loot.startsWith(lastText) || loot.replace("minecraft:","").startsWith(lastText)) {
						out.push({
							text: loot,
							type: "loot",
				...
				for(var slot of slots["inventory"]) {
				...
				for(var slot of slots["scoreboard"]) {
					if(slot.startsWith(lastText)) {
						out.push({
							text: slot,
							type: "slot",
				...
				for(var slot of slots["objective"]) {
					if(slot.startsWith(lastText)) {
						out.push({
							text: slot,
							type: "slot",
