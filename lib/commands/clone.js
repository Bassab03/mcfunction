...
class CloneCommand12 extends BaseCommand {
  ...
  handleSuggestions(tokens) {
    switch(tokens.length) {
      ...
      case 3:
      case 4: {
        return [new CoordinateOutput(["x1", "y1", "z1"].slice(tokens.length - 2), "~", 5 - tokens.length, "First corner to start cloning from")];
      }
      case 5:
      case 6:
      case 7: {
        return [new CoordinateOutput(["x2", "y2", "z2"].slice(tokens.length - 2), "~", 8 - tokens.length, "Second corner to start cloning from")];
      }
      case 8:
      case 9:
      case 10: {
        return [new CoordinateOutput(["x", "y", "z"].slice(tokens.length - 2), "~", 11 - tokens.length, "Location to place the cloned structure at. Places starting at the bottom, northwest corner.")];
      }
      case 11: {
        return [
          new EnumOutput(
