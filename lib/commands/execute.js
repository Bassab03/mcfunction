...
class ExecuteCommand13 extends BaseCommand {
  ...
  handleSuggestions(tokens) {
    ...
    for (let i = tokens.length - 1; i >= 1; i++) {
      currentSubCommand.splice(0, 0, tokens[i]);
      if (this.isSubCommandDone(currentSubCommand)) {
        break;
      } 
    }
    switch (currentSubCommand.length) {
      case 0:
      case 1: {
  ...
  /**
   * ifUnlessDone - Code for if/unless conditionals
   * 
   * @param {Token[]} command 
   */
